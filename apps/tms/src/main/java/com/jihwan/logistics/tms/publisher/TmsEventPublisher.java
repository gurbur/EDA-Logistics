package com.jihwan.logistics.tms.publisher;
...
public class TmsEventPublisher {
    ...
    public TmsEventPublisher() throws JCSMPException {
    ...
    public void publishAllocationResult(String orderId, String result, String reason) {
        try {
            String topicStr = String.format("TOPIC/JIHWAN_LOGIS/TMS/ALLOCATION_RESULT/%s/%s",
            ...
            System.out.printf("[TMS ALLOCATION %s] Published to [%s]: %s%n", result.toUpperCase(), topicStr, msg.getText());
        } catch (Exception e) {
            System.err.println("Exception during TMS allocation publish: " + e.getMessage());
        }
    }
}
